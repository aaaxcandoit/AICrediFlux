package aicrediflux.token.service.payment.waffopancake;
...
import aicrediflux.token.BaseIntegrationTest;
...
import aicrediflux.token.service.payment.waffopancake.pojo.WaffoPancakeCatalogProduct;
import aicrediflux.token.service.payment.waffopancake.pojo.WaffoPancakeCatalogStore;
 ...
 * 通过 aicrediflux Header（登录后从响应体取 token）传递鉴权，复用 WaffoPancakeOptionIT 的 root 登录模式。
 */
...
class WaffoPancakePairIT extends BaseIntegrationTest {
    ...
    void subProductOptions_Success() {
        ...
        optHeaders.set("aicrediflux", saToken);
    ...
    private ResponseEntity<Map> apiPost(String path, Map<String, Object> body) {
        ...
        headers.set("aicrediflux", saToken);
