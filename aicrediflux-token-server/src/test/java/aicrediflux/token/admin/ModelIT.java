package aicrediflux.token.admin;
...
import aicrediflux.token.factory.ModelFactory;
 ...
 * 鉴权通过 aicrediflux Header（登录后从响应体取 token）+ aicrediflux-user-id Header，复用 ChannelIT 鉴权模式。
 */
...
public class ModelIT extends BaseIntegrationTest {
    ...
    /**
     * PUT 请求（带 JSON body + 鉴权头：aicrediflux + aicrediflux-user-id）。
     */
    ...
    /**
     * 通用请求（带鉴权头：aicrediflux + aicrediflux-user-id）。
     */
