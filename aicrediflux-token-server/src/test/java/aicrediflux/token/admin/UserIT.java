...
import aicrediflux.token.factory.UserFactory;
 ...
 * 鉴权通过 aicrediflux Header（登录后从响应体取 token）+ aicrediflux-user-id Header，复用 ChannelIT 模式。管理端点位于 AdminController（/api/user/*）。
 */
...
public class UserIT extends BaseIntegrationTest {
    ...
    void setUpAdmin() throws Exception {
        ...
        // 从 JSON 响应体取 token（后端 login 接口直接返回 token 值，通过 aicrediflux Header 传递）
