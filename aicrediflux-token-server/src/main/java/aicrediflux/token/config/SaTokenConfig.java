...
public class SaTokenConfig implements WebMvcConfigurer {
    ...
    public void addInterceptors(InterceptorRegistry registry) {
        registry.addInterceptor(new SaInterceptor())
                ...
                // SSE 会在 Servlet 异步分发阶段再次进入 MVC 拦截器，此时 Sa-Token 的线程上下文已脱离原请求。
                // 登录态仍由 AuthFilter 校验并注入 request attribute，AgentController 再做 run 归属校验。
                .excludePathPatterns("/api/agent/runs/*/events");
    }
}
