...
public final class RelayUtils {
    ...
    public static String getFullRequestURL(String baseURL, String requestURL, int channelType) {
        ...
        if (baseURL.startsWith("https://gateway.ai.cloudflare.com")) {
            ...
            if (channelType == aicrediflux.token.constant.ChannelConstants.CHANNEL_TYPE_OPENAI) {
                fullURL = baseURL + requestURL.replaceFirst("^/v1", "");
            } else if (channelType == aicrediflux.token.constant.ChannelConstants.CHANNEL_TYPE_AZURE) {
    ...
    public static byte[] removeDisabledFields(byte[] jsonData,
        ...
        // PassThrough 模式：跳过移除
        boolean passThroughGlobal = aicrediflux.token.config.model.GlobalModelSettingConfig.getInstance().isPassThroughRequestEnabled();
