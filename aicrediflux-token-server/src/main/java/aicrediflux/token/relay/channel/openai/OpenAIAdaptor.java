package aicrediflux.token.relay.channel.openai;
...
import aicrediflux.token.config.ReasoningSuffixConfig;
import aicrediflux.token.constant.ChannelConstants;
import aicrediflux.token.relay.channel.ai360.AI360Constant;
import aicrediflux.token.relay.channel.openrouter.OpenRouterConstant;
import aicrediflux.token.relay.channel.xinference.XinferenceConstant;
import aicrediflux.token.pojo.dto.*;
import aicrediflux.token.pojo.dto.OpenAIResponseDTO.OpenAITextResponse;
import aicrediflux.token.pojo.dto.OpenAIResponseDTO.OpenAITextResponseChoice;
import aicrediflux.token.relay.channel.ApiRequestExecutor;
import aicrediflux.token.relay.channel.IAdaptor;
import aicrediflux.token.relay.channel.IAdaptor.DoResponseResult;
import aicrediflux.token.relay.common.RelayInfo;
import aicrediflux.token.relay.common.RelayUtils;
...
import aicrediflux.token.service.ConvertService;

import static aicrediflux.token.relay.handler.CompatibleHandler.newApiError;
...
public class OpenAIAdaptor implements IAdaptor {    /** 渠道类型（用于模型列表/渠道名分发） */
    ...
    public Map<String, String> setupRequestHeader(RelayInfo info) throws Exception {
        ...
        if (this.channelType == ChannelConstants.CHANNEL_TYPE_OPENROUTER) {
            headers.putIfAbsent("HTTP-Referer", "https://token.aicrediflux.cc");
            headers.putIfAbsent("X-OpenRouter-Title", "AICrediFlux");
        }
    ...
    private DoResponseResult handleNonStreamResponse(
        ...
        aicrediflux.token.pojo.dto.Usage usage = convertUsage(simpleResponse.getUsage());
        ...
        if (usage == null || usage.getPromptTokens() == 0) {
            ...
            usage = new aicrediflux.token.pojo.dto.Usage();
    ...
    private DoResponseResult handleStreamResponse(
        ...
        int[] toolCount = {0};
        aicrediflux.token.pojo.dto.Usage[] usageHolder = {new aicrediflux.token.pojo.dto.Usage()};
        ...
        // 如果没有从流中提取到 usage，估算
        aicrediflux.token.pojo.dto.Usage usage = usageHolder[0];
    ...
    /**
     * 转换 OpenAIResponseDTO.Usage → aicrediflux.token.pojo.dto.Usage
     */
    private static aicrediflux.token.pojo.dto.Usage convertUsage(OpenAIResponseDTO.Usage source) {
        if (source == null) return null;
        aicrediflux.token.pojo.dto.Usage target = new aicrediflux.token.pojo.dto.Usage();
