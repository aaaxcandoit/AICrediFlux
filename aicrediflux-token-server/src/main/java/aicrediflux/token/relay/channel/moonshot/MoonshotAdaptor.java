package aicrediflux.token.relay.channel.moonshot;

import lombok.extern.slf4j.Slf4j;
import aicrediflux.token.constant.ChannelConstants;
import aicrediflux.token.constant.ChannelConstants.ChannelSpecialBase;
...
public class MoonshotAdaptor extends OpenAIAdaptor {
    ...
    public String getRequestURL(RelayInfo info) throws Exception {
        ...
        if (relayMode == aicrediflux.token.relay.constant.RelayModeEnum.RERANK) {
            return baseURL + "/v1/rerank";
        } else if (relayMode == aicrediflux.token.relay.constant.RelayModeEnum.EMBEDDINGS) {
            return baseURL + "/v1/embeddings";
        } else if (relayMode == aicrediflux.token.relay.constant.RelayModeEnum.COMPLETIONS) {
            return baseURL + "/v1/completions";
        } else if (relayMode == aicrediflux.token.relay.constant.RelayModeEnum.CHAT_COMPLETIONS) {
