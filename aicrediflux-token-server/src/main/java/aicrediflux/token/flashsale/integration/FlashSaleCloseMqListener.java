...
@RocketMQMessageListener(
        topic = "${aicrediflux.flash-sale.topics.close:mr-flash-sale-delay-close}",
        consumerGroup = "${aicrediflux.flash-sale.consumer-groups.close:aicrediflux-flash-sale-close}")
public class FlashSaleCloseMqListener implements RocketMQListener<String> {
    private final FlashSaleEventRouter router;
    private final FlashSaleProperties properties;
    @Override public void onMessage(String payload) { router.route(properties.getTopics().getClose(), payload); }
}
