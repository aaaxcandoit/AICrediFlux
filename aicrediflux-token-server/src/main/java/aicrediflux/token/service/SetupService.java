...
import aicrediflux.token.constant.SetupConstants;
import aicrediflux.token.mapper.SetupMapper;
import aicrediflux.token.mapper.UserMapper;
import aicrediflux.token.pojo.entity.Setup;
import aicrediflux.token.pojo.entity.User;
import aicrediflux.token.pojo.ipo.SetupIPO;
import aicrediflux.token.pojo.vo.SetupStatusVO;
...
public class SetupService {
    ...
    /**
     * auto 模式初始化，由 {@link aicrediflux.token.config.RootUserInitializer} 启动时调用。
     * <p>
    ...
    public void initializeAuto() {
        ...
        if (SetupConstants.MODE_INTERACTIVE.equalsIgnoreCase(mode)) {
            log.warn("[Setup] 系统未初始化，AICREDIFLUX_INIT_MODE=interactive，等待前端 SetupWizard 通过 POST /api/setup 完成");
            return;
        }
        ...
        if (!rootExists) {
            if (passwordFromEnv) {
                log.warn("[Setup] auto 模式初始化完成，root 账号: {}（密码由环境变量 AICREDIFLUX_INIT_ROOT_PASSWORD 提供）", username);
            } else {
