...
public class PlaygroundController {
    ...
    public void chatCompletions(HttpServletRequest request, HttpServletResponse response) {
        RelayInfo info = relayInfoBuilder.buildPlayground(request, DispatchContext.playground(
                toInt(request.getAttribute("id")),
                resolveGroup(request),
                (String) request.getAttribute("request_id")));
        info.setRelayMode(RelayModeEnum.CHAT_COMPLETIONS);
        modelDispatchService.dispatchRelay(request, response, info, "openai");
    }

    private String resolveGroup(HttpServletRequest req) {
