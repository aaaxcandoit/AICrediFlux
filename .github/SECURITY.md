...
> [!IMPORTANT]
> **Bulk Reporting Policy:** If you need to submit multiple vulnerability reports in bulk, **you must contact us first** ([support@yaoshu.cc](mailto:support@yaoshu.cc)) to coordinate the submission process. Uncoordinated bulk submissions have caused significant disruption to our team, and we will take the following actions:
>
...
To report a security issue, please use the GitHub Security Advisories tab to "[Open a draft security advisory](https://github.com/aicrediflux-open/aicrediflux/security/advisories/new)". This is the preferred method as it provides a built-in private communication channel.

Alternatively, you can report via email:

- **Email:** [support@yaoshu.cc](mailto:support@yaoshu.cc)
...
When deploying and using AICrediFlux, we recommend following these security best practices:
...
Please ensure the following security-related settings are properly configured:

- `SESSION_SECRET` — Use a strong random string (required for consistent login state across multi-instance deployments)
- `CRYPTO_SECRET` — Use a strong random string (required for shared Redis data encryption across instances)
- `spring.datasource.url` — Ensure database connection uses secure configuration (TLS, dedicated user, strong password)
- `spring.datasource.username` — Use a dedicated database user with least privilege
- `spring.datasource.password` — Use a strong password, never hardcode in source; inject via environment variable `SPRING_DATASOURCE_PASSWORD`
- `spring.data.redis.host` — Ensure Redis is not exposed to the public internet
- `spring.data.redis.password` — Use a strong Redis password; inject via environment variable `SPRING_REDIS_PASSWORD`

For detailed configuration instructions, please refer to the [README](https://github.com/aicrediflux-open/aicrediflux#readme).
