...
export const DEFAULT_SYSTEM_NAME = 'AICrediFlux'
...
function mapStatusDataToConfig(
  ...
  return {
    // 固定项目品牌，避免历史部署配置或旧 localStorage 覆盖前端品牌资源。
...
const STORAGE_KEY = 'aicrediflux_system_status_v2'
...
export const useSystemConfigStore = defineStore('system-config', () => {
  ...
  const systemName = ref<string>(DEFAULT_SYSTEM_NAME)
  const logo = ref<string>(DEFAULT_LOGO)
