...
const mockedGetEnabledModels = vi.mocked(getEnabledModels)
...
function mountForm(overrides: Record<string, string> = {}, enabledModels: unknown[] = []) {
  mockedGetEnabledModels.mockResolvedValue(enabledModels as never)
  return mount(ModelRatioForm, {
    ...
    global: {
      ...
      stubs: {
        ...
        ElTable: {
          props: ['data'],
          template: '<div class="el-table-stub"><div v-for="row in data" :key="row.name" class="el-table-stub__row">{{ row.name }}</div></div>',
        },
      },
    },
  })
...
describe('ModelRatioForm - enabled models contract', () => {
  it('后端返回字符串数组时仍显示未设置模型名', async () => {
    const wr = mountForm({
      modelRatio: '{}', modelPrice: '{}', completionRatio: '{}', cacheRatio: '{}',
      createCacheRatio: '{}', imageRatio: '{}', audioRatio: '{}', audioCompletionRatio: '{}',
    }, ['deepseek-v4-flash'])
    await nextTick()
    await nextTick()
    expect(wr.html()).toContain('deepseek-v4-flash')
  })
})